    ...
    with open(path, encoding="utf-8") as fh:
        for line in fh:
            ...
            entry = {
                ...
                "url": url,
                "description": desc,
                "category": {"id": "/".join(path_ids), "path": path_titles},
                "github": None,
            }
...
def apply_annotations(entries, overrides):
    for e in entries:
        top_cat = e["category"]["path"][0] if e["category"]["path"] else ""
...
def main():
    ...
    meta = fetch_repo_metadata(repos, token, sleep_s=2.0 if token else 6.5)
    ...
    cat_counts = {}
    for e in entries:
        cat_counts[e["category"]["id"]] = cat_counts.get(e["category"]["id"], 0) + 1
    for c in categories:
    ...
    doc = {
        ...
        "stats": {
            "entries": len(entries),
            "github_repos": sum(1 for e in entries if e["github"]),
